...
int cast_spell(prompt, item_val, sn, sc) char *prompt;
int item_val;
int *sn, *sc;
{
  int32u j;
    ...
    if (i == TV_GOLD) {
      ...
      {                           /* Okay,  pick it up      */
        ...
        if (pickup && !inven_check_weight(i_ptr)) {
          ...
          /* change the period to a question mark */
          tmp_str[strlen(tmp_str) - 1] = '?';
          (void)sprintf(out_val, "Exceed your weight limit to pick up %s",
                        tmp_str);
          pickup = get_check(out_val);
        }
...
void fix1_delete_monster(j) int j;
{
  register monster_type *m_ptr;
...
void fix2_delete_monster(j) int j;
{
  register monster_type *m_ptr;
