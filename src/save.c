...
int save_char(always_ask) int always_ask;
{
  ...
  if (!already_set || always_ask) {
    ...
    sfposition(vrefnum);
    proceed = doputfile(death ? "Save memories as:" : "Save game as:", savefile,
                        &vrefnum);
  } else
    proceed = TRUE;
...
static void wr_short(s) int16u s;
{
  xor_byte ^= (s & 0xFF);
  (void)putc((int)xor_byte, fileptr);
...
static void wr_long(l) register int32u l;
{
  xor_byte ^= (l & 0xFF);
  (void)putc((int)xor_byte, fileptr);
...
static void wr_string(str) register char *str;
{
  DEBUG(char *s = str);
...
static void wr_shorts(s, count) int16u *s;
register int count;
{
  register int i;
...
static void wr_item(item) register inven_type *item;
{
...
static void wr_monster(mon) register monster_type *mon;
{
...
static void rd_short(ptr) int16u *ptr;
{
  int8u c;
  int16u s;
...
static void rd_string(str) char *str;
{
  register int8u c;
...
static void rd_shorts(ptr, count) int16u *ptr;
register int count;
{
  register int i;
  register int16u *sptr;
...
static void rd_item(item) register inven_type *item;
{
  DEBUG(fprintf(logfile, "ITEM:\n"));
  rd_short(&item->index);
  rd_byte(&item->name2);
...
static void rd_monster(mon) register monster_type *mon;
{
  DEBUG(fprintf(logfile, "MONSTER:\n"));
...
void set_fileptr(file) FILE *file;
{ fileptr = file; }

void wr_highscore(score) high_scores *score;
{
  DEBUG(logfile = fopen("IO_LOG", "a"));
...
void rd_highscore(score) high_scores *score;
{
  DEBUG(logfile = fopen("IO_LOG", "a"));
