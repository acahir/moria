...
int typ, num;
{
  ...
  for (k = 0; k < num; k++) {
    ...
    while ((!(*alloc_set)(cave[i][j].fval)) || (cave[i][j].tptr != 0) ||
           (i == char_row && j == char_col));
...
static void prt_lnum(header, num, row, column) char *header;
int32 num;
int row, column;
{
  vtype out_val;
...
static void prt_7lnum(header, num, row, column) char *header;
int32 num;
int row, column;
{
  vtype out_val;
...
static void prt_long(num, row, column) int32 num;
int row, column;
{
  vtype out_val;
...
void prt_gold() { prt_long(py.misc.au, 20, STAT_COLUMN + 6); }
...
int8u modify_stat(stat, amount) int stat;
int16 amount;
{
  register int loop, i;
...
void bst_stat(stat, amount) int stat, amount;
{
#ifdef ATARIST_MWC
  int32u holder;
#endif
...
void put_misc2() {
  ...
  if (m_ptr->lev >= MAX_PLAYER_LEVEL)
    ...
    prt_7lnum("Exp to Adv.",
              (int32)(player_exp[m_ptr->lev - 1] * m_ptr->expfact / 100), 12,
              28);
...
void take_one_item(s_ptr, i_ptr) register inven_type *s_ptr, *i_ptr;
{
  *s_ptr = *i_ptr;
...
int inven_check_num(t_ptr) register inven_type *t_ptr;
{
  register int i;
...
int inven_check_weight(i_ptr) register inven_type *i_ptr;
{
...
int inven_carry(i_ptr) register inven_type *i_ptr;
{
  ...
  for (locn = 0;; locn++) {
    ...
    else if ((typ == t_ptr->tval && subt < t_ptr->subval && always_known1p) ||
             (typ > t_ptr->tval)) {
  ...
  if (spell_learned != 0) {
    ...
    if (p_ptr->mana != new_mana) {
      if (p_ptr->mana != 0) {
        ...
        value = (((long)p_ptr->cmana << 16) + p_ptr->cmana_frac) / p_ptr->mana *
                new_mana;
...
void calc_hitpoints() {
  ...
  if ((hitpoints != p_ptr->mhp) && (p_ptr->mhp != 0)) {
    /* change current hit points proportionately to change of mhp,
       divide first to avoid overflow, little loss of accuracy */
    value =
        (((long)p_ptr->chp << 16) + p_ptr->chp_frac) / p_ptr->mhp * hitpoints;
    p_ptr->chp = value >> 16;
...
void insert_lnum(object_str, mtc_str, number, show_sign) char *object_str;
register char *mtc_str;
int32 number;
int show_sign;
{
...
register int *y, *x;
{
  ...
  bool = FALSE;
  if ((new_row >= 0) && (new_row < cur_height) && (new_col >= 0) &&
      (new_col < cur_width)) {
    *y = new_row;
    *x = new_col;
    bool = TRUE;
