...
int show_inven(r1, r2, weight, col, mask) register int r1, r2;
int weight, col;
char *mask;
{
  register int i;
  ...
  for (i = r1; i <= r2; i++) {
    if (mask == CNIL || mask[i]) {
      ...
      if (weight) {
        ...
        (void)sprintf(tmp_val, "%3d.%d lb", (total_weight) / 10,
                      (total_weight) % 10);
...
char *describe_use(i) register int i;
{
  register char *p;
  ...
  for (i = INVEN_WIELD; i < INVEN_ARRAY_SIZE; i++) /* Range of equipment */
  {
    ...
    if (i_ptr->tval != TV_NOTHING) {
      ...
      if (weight) {
        ...
        (void)sprintf(prt2, "%3d.%d lb", (total_weight) / 10,
                      (total_weight) % 10);
        prt(prt2, line + 1, 71);
      }
      line++;
...
static void inven_screen(new_scr) int new_scr;
{
  register int line;
  ...
  if (new_scr != scr_state) {
    ...
    case WEAR_SCR:
      scr_left =
          show_inven(wear_low, wear_high, show_weight_flag, scr_left, CNIL);
...
void inven_command(command) char command;
{
  ...
  do {
    ...
    case 'x':
...
#endif
      {
        ...
        (void)sprintf(prt2, "The %s you are wielding appears to be cursed.",
                      prt1);
        msg_print(prt2);
      } else {
    ...
    while (selecting && free_turn_flag) {
      ...
      if (from > to)
        ...
        if (scr_state == BLANK_SCR)
          ...
          disp = "";
        (void)sprintf(
            prt1, "(%c-%c%s%s%s, space to break, ESC to exit) %s which one?",
            from + 'a', to + 'a', disp, swap,
            ((command == 'w' || command == 'd') ? ", 0-9" : ""), prompt);
        /* Abort everything. */
        ...
        /* Swap screens (for drop) */
          ...
          if ((which >= '0') && (which <= '9') && (command != 'r') &&
              (command != 't')) {
            /* look for item whose inscription matches "which" */
            register int m;
            for (m = from; m <= to && ((inventory[m].inscrip[0] != which) ||
    ...
    if (which == ESCAPE || scr_state == BLANK_SCR)
      ...
      if (scr_state == INVEN_SCR) {
        if (!show_weight_flag || inven_ctr == 0)
          (void)sprintf(
              prt1, "You are carrying %d.%d pounds. In your pack there is %s",
              inven_weight / 10, inven_weight % 10,
              (inven_ctr == 0 ? "nothing." : "-"));
        else
          (void)sprintf(prt1, "You are carrying %d.%d pounds. Your capacity is "
                              "%d.%d pounds. %s",
                        inven_weight / 10, inven_weight % 10,
    ...
    do {
      ...
      if (full)
        (void)sprintf(out_val, "(%s: %c-%c,%s%s / for %s, or ESC) %s",
                      ...
                      (i_scr > 0 ? " 0-9," : ""), (redraw ? "" : " * to see,"),
                      (i_scr > 0 ? "Equip" : "Inven"), pmt);
      else
        (void)sprintf(out_val, "(Items %c-%c,%s%s ESC to exit) %s", i + 'a',
                      j + 'a', (i_scr > 0 ? " 0-9," : ""),
      ...
      do {
        ...
        default:
          ...
          /* look for item whose inscription matches "which" */
          {
            register int m;
            for (m = i;
                 (m < INVEN_WIELD) && ((inventory[m].inscrip[0] != which) ||
                                       (inventory[m].inscrip[1] != '\0'));
                 m++)
              ;
            if (m < INVEN_WIELD)
    ...
    attack_type;
{
  ...
  die = randint(20);
  if ((die != 1) &&
      ((die == 20) || ((i > 0) && (randint(i) > ac)))) /* normal hit */
    return TRUE;
  else
    return FALSE;
}
