...
/* Look at everything within a cone of vision between two ray
  ...
  ...|../.....___.................... parameter y (look at things in the
     ...
     |
     | */
static int look_ray(y, from, to) int y, from, to;
{
...
static int look_see(x, y, transparent) register int x, y;
int *transparent;
{
  ...
  *transparent = c_ptr->fval <= MAX_OPEN_SPACE;
  ...
  if (gl_rock == 0 && c_ptr->cptr > 1 && m_list[c_ptr->cptr].ml) {
    ...
    (void)sprintf(out_val, "%s %s %s. [(r)ecall]", dstring,
                  is_a_vowel(c_list[j].name[0]) ? "an" : "a", c_list[j].name);
    dstring = "It is on";
    prt(out_val, 0, 0);
...
static void inven_throw(item_val, t_ptr) int item_val;
inven_type *t_ptr;
{
  register inven_type *i_ptr;
#ifdef ATARIST_MWC
...
/* Obtain the hit and damage bonuses and the maximum distance for a
   thrown missile. */
static void facts(i_ptr, tbth, tpth, tdam, tdis) register inven_type *i_ptr;
...
static void drop_throw(y, x, t_ptr) int y, x;
inven_type *t_ptr;
{
  register int i, j, k;
  int flag, cur_pos;
      ...
      do {
        ...
        if ((c_ptr->fval <= MAX_OPEN_SPACE) && (!flag)) {
          if (c_ptr->cptr > 1) {
            ...
            if (!m_ptr->ml)
              tbth = (tbth / (cur_dis + 2)) -
                     (py.misc.lev * class_level_adj[py.misc.pclass][CLA_BTHB] /
                      2) -
                     (tpth * (BTH_PLUS_ADJ - 1));
...
static void py_bash(y, x) int y, x;
{
  ...
  if (test_hit(base_tohit, (int)py.misc.lev, (int)py.stats.use_stat[A_DEX],
    ...
    k = critical_blow(
        (int)(inventory[INVEN_ARM].weight / 4 + py.stats.use_stat[A_STR]), 0, k,
        CLA_BTH);
    k += py.misc.wt / 60 + 3;
    if (k < 0)
      k = 0;
