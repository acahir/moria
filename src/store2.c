...
static char *comment2a[3] = {"%A2 is my final offer; take it or leave it.",
...
static char *comment3a[3] = {"I'll pay no more than %A1; take it or leave it.",
                             "You'll get no more than %A1 from me.",
                             "%A1 and that's final."};

static char *comment3b[15] = {"%A2 for that piece of junk?  No more than %A1.",
...
static char *comment6[5] = {"I must have heard you wrong.", "What was that?",
...
static void prt_comment1() { msg_print(comment1[randint(14) - 1]); }
...
static void prt_comment3(offer, asking, final) int32 offer, asking;
int final;
{
  vtype comment;
...
static void prt_comment5() { msg_print(comment5[randint(10) - 1]); }

static void prt_comment6() { msg_print(comment6[randint(5) - 1]); }
...
register int i, j;
{
  ...
  *com_val = -1;
  flag = FALSE;
  (void)sprintf(out_val, "(Items %c-%c, ESC to exit) %s", i + 'a', j + 'a',
                pmt);
...
static int get_haggle(comment, new_offer, num_offer) char *comment;
int32 *new_offer;
int num_offer;
{
  register int32 i;
  ...
  do {
    ...
    if (num_offer && last_store_inc != 0) {
      (void)sprintf(default_offer, "[%c%d] ", (last_store_inc < 0) ? '-' : '+',
                    abs(last_store_inc));
...
static int receive_offer(store_num, comment, new_offer, last_offer, num_offer,
                         factor) int store_num;
char *comment;
  ...
  *price = 0;
  ...
  do {
    do {
      ...
      purchase = receive_offer(store_num, "What do you offer? ", &new_offer,
                               last_offer, num_offer, 1);
      if (purchase != 0)
        flag = TRUE;
      else {
...
the price.");
    ...
    do {
      do {
        ...
        (void)sprintf(out_val, "%s :  %d", comment, cur_ask);
        put_buffer(out_val, 1, 0);
        sell = receive_offer(store_num, "What price do you ask? ", &new_offer,
                             last_offer, num_offer, -1);
        if (sell != 0)
          flag = TRUE;
        else {
