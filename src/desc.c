...
int16 object_offset(t_ptr) inven_type *t_ptr;
{
  switch (t_ptr->tval) {
  case TV_AMULET:
    return (0);
...
int known1_p(i_ptr) inven_type *i_ptr;
{
  int16 offset;
  int8u indexx;
...
int known2_p(i_ptr) inven_type *i_ptr;
{ return (i_ptr->ident & ID_KNOWN2); }

void clear_known2(i_ptr) inven_type *i_ptr;
{ i_ptr->ident &= ~ID_KNOWN2; }

void clear_empty(i_ptr) inven_type *i_ptr;
{ i_ptr->ident &= ~ID_EMPTY; }

void store_bought(i_ptr) inven_type *i_ptr;
{
  i_ptr->ident |= ID_STOREBOUGHT;
  known2(i_ptr);
}

int store_bought_p(i_ptr) inven_type *i_ptr;
...
void sample(i_ptr) inven_type *i_ptr;
{
  int16 offset;
  int8u indexx;
...
void objdes(out_val, i_ptr, pref) char *out_val;
  ...
  if (!pref) {
    ...
    if (known2_p(i_ptr)) {
      ...
      if (i_ptr->ident & ID_SHOW_HITDAM)
        (void)sprintf(tmp_str, " (%c%d,%c%d)", (i_ptr->tohit < 0) ? '-' : '+',
                      abs(i_ptr->tohit), (i_ptr->todam < 0) ? '-' : '+',
                      abs(i_ptr->todam));
      else if (i_ptr->tohit != 0)
        (void)sprintf(tmp_str, " (%c%d)", (i_ptr->tohit < 0) ? '-' : '+',
                      abs(i_ptr->tohit));
      else if (i_ptr->todam != 0)
        (void)sprintf(tmp_str, " (%c%d)", (i_ptr->todam < 0) ? '-' : '+',
                      abs(i_ptr->todam));
      else
        tmp_str[0] = '\0';
    ...
    if (i_ptr->ac != 0 || (i_ptr->tval == TV_HELM)) {
      ...
      if (known2_p(i_ptr)) {
        /* originally used %+d, but several machines don't support it */
        (void)sprintf(tmp_str, ",%c%d", (i_ptr->toac < 0) ? '-' : '+',
                      abs(i_ptr->toac));
        (void)strcat(tmp_val, tmp_str);
      }
      (void)strcat(tmp_val, "]");
    ...
    if (p1_use == LIGHT)
      ...
      if (p1_use == Z_PLUSSES)
        /* originally used %+d, but several machines don't support it */
        (void)sprintf(tmp_str, " (%c%d)", (i_ptr->p1 < 0) ? '-' : '+',
                      abs(i_ptr->p1));
...
void invcopy(to, from_index) register inven_type *to;
int from_index;
{
