  ...
  while (!feof(highscore_fp)) {
    ...
    while (!feof(highscore_fp) && i < 21) {
      ...
      if (!show_player ||
        ...
        (void)sprintf(string, "%-4d%8d %-19.19s %c %-10.10s %-7.7s%3d %-22.22s",
...
/* write out new_entry at end of file */
    ...
    while (!feof(highscore_fp)) {
      ...
      (void)fseek(highscore_fp, (long)ftell(highscore_fp) -
                                    sizeof(high_scores) - sizeof(char),
                  L_SET);
#else
      ...
      (void)fseek(highscore_fp,
                  -(off_t)sizeof(high_scores) - (off_t)sizeof(char), L_INCR);
#endif
      wr_highscore(&entry);
