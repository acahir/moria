...
#ifdef ATARIST_MWC
    if ((py.flags.status & (holder = PY_PARALYSED)) && (py.flags.paralysis < 1))
#else
    ...
    if (((turn & 0xF) == 0) && (f_ptr->confused == 0) &&
      ...
      for (i = 0; i < INVEN_ARRAY_SIZE; i++) {
        ...
        if ((i_ptr->tval != TV_NOTHING) && enchanted(i_ptr) &&
          ...
          (void)sprintf(tmp_str, "There's something about what you are %s...",
                        describe_use(i));
          disturb(0, 0);
          msg_print(tmp_str);
      ...
      do {
        ...
        if (find_flag) {
          ...
          if (command_count > 0) {
            ...
            if ((rogue_like_commands && command >= '0' && command <= '9') ||
...
static char original_commands(com_val) char com_val;
{
  int dir_val;
...
static int valid_countcommand(c) char c;
{
  switch (c) {
  case 'Q':
  case CTRL('W'):
...
static int enchanted(t_ptr) register inven_type *t_ptr;
{
#ifdef ATARIST_MWC
  int32u holder;
#endif
