...
void lower_monster_name(m_name, m_ptr, r_ptr) char *m_name;
monster_type *m_ptr;
creature_type *r_ptr;
{
  if (!m_ptr->ml)
            ...
            if (m_ptr->hp < 0) {
              ...
              if (m_ptr->ml) {
                ...
                c_recall[m_ptr->mptr].r_cmove =
                    treas | (c_recall[m_ptr->mptr].r_cmove & ~holder);
#else
...
int recharge(num) register int num;
{
  int i, j, item_val;
  register int res;
  register inven_type *i_ptr;
...
void earthquake() {
  ...
  for (i = char_row - 8; i <= char_row + 8; i++)
    for (j = char_col - 8; j <= char_col + 8; j++)
      if (((i != char_row) || (j != char_col)) && in_bounds(i, j) &&
          (randint(8) == 1)) {
        c_ptr = &cave[i][j];
        if (c_ptr->tptr != 0)
          (void)delete_object(i, j);
...
static void replace_spot(y, x, typ) int y, x, typ;
{
  register cave_type *c_ptr;
