...
void reset_seed() { set_rnd_seed(old_seed); }
...
int pdamroll(array) int8u *array;
...
int los(fromY, fromX, toY, toX) int fromY, fromX, toY, toX;
{
...
unsigned char loc_symbol(y, x) int y, x;
{
  ...
  if ((cave_ptr->cptr == 1) && (!find_flag || find_prself))
    ...
    return '.';
#endif
  } else if (cave_ptr->fval == GRANITE_WALL ||
             cave_ptr->fval == BOUNDARY_WALL || highlight_seams == FALSE) {
#ifdef MSDOS
    return wallsym;
#else
#ifndef ATARI_ST
...
int get_mons_num(level) int level;
{
...
int m_bonus(base, max_std, level) int base, max_std, level;
{
