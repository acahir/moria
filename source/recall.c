...
static char *desc_atype[] = {"do something undefined",
                             "attack",
                             "weaken",
                             "confuse",
                             "terrify",
                             ...
                             "absorb light",
                             "absorb charges"};
static char *desc_amethod[] = {"make an undefined advance",
                               "hit",
                               "bite",
                               "claw",
                               "sting",
                               ...
                               "drool",
                               "insult"};
static char *desc_howmuch[] = {" not at all", " a bit",    "",
                               " quite",      " very",     " most",
                               " highly",     " extremely"};

static char *desc_move[] = {"move invisibly",     "open doors",
                            "pass through walls", "kill weaker creatures",
                            "pick up objects",    "breed explosively"};
static char *desc_spell[] = {"teleport short distances",
                             ...
                             "unknown 1",
                             "unknown 2"};
static char *desc_breath[] = {"lightning", "poison gases", "acid", "frost",
                              "fire"};
...
int roff_recall(mon_num) int mon_num;
{
  ...
  if (mp->r_deaths) {
    ...
    if (mp->r_kills == 0)
      ...
      (void)sprintf(temp, "at least %d of the beasts %s been exterminated.",
  ...
  if (mp->r_kills) {
    ...
    j = (((long)cp->mexp * cp->level % py.misc.lev) * (long)1000 / py.misc.lev +
         5) /
        10;
  ...
  if (knowarmor(cp->level, mp->r_kills)) {
    ...
    (void)sprintf(temp, " and a%s life rating of %dd%d.",
                  ((cp->cdefense & CD_MAX_HP) ? " maximized" : ""), cp->hd[0],
                  cp->hd[1]);
    roff(temp);
  }
...
static void roff(p) register char *p;
{
  register char *q, *r;
