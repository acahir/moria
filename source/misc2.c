...
#ifdef ATARIST_MWC
          t_ptr->flags |=
              (holder = TR_RES_LIGHT | TR_RES_COLD | TR_RES_ACID | TR_RES_FIRE);
#else
          t_ptr->flags |=
...
#else
          t_ptr->flags |=
              (TR_FFALL | TR_RES_LIGHT | TR_SEE_INVIS | TR_FREE_ACT |
               TR_RES_COLD | TR_RES_ACID | TR_RES_FIRE | TR_REGEN | TR_STEALTH);
#endif
          t_ptr->tohit += 3;
          t_ptr->todam += 3;
      ...
      if (magik(special)) {
        if (t_ptr->subval < 6) {
          ...
          case 1:
            ...
            t_ptr->p1 = randint(3);
#ifdef ATARIST_MWC
            t_ptr->flags |= (holder = TR_FREE_ACT | TR_CON | TR_DEX | TR_STR);
#else
